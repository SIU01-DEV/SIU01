  ...
  implements IAsistenciasEscolaresIDB
{
    ...
    try {
      if (nivel !== NivelEducativo.PRIMARIA) {
        return this.crearResultadoError(
          "No tiene permisos para consultar asistencias de Secundaria."
        );
      }
  ...
  private async procesarRespuestaAPI(
    ...
    try {
      ...
      if (!nivelGrado) {
        throw new Error("No se pudo determinar nivel y grado");
      }
  ...
  private async consultarAPIAsistenciasDiaActual(
    ...
    try {
      const tipoAsistencia = TipoAsistencia.ParaEstudiantesPrimaria;
  ...
  private async debeConsultarDiaActual(idAula: string): Promise<boolean> {
    try {
      ...
      const { HandlerProfesorPrimariaAsistenciaResponse } = await import(
        "@/lib/utils/local/db/models/DatosAsistenciaHoy/handlers/HandlerProfesorPrimariaAsistenciaResponse"
      );
      const handler =
        (await new DatosAsistenciaHoyIDB().getHandler()) as InstanceType<
          typeof HandlerProfesorPrimariaAsistenciaResponse
        >;
      ...
      const horarioEscolar = handler.getHorarioEscolarPrimaria();
