...
const ProfesoresSecundaria = () => {
  ...
  const [profesores, setProfesores] = useState<ProfesorSecundariaListItem[]>(
    [],
  );
    ...
    <div className="w-full min-h-full">
      ...
      <h1 className=" text-[2rem] flex justify-between items-center -border-2 gap-3 font-semibold text-center leading-8 sxs-only:text-[1.55rem] xs-only:text-[1.65rem] sm-only:text-[1.75rem] flex-wrap my-4 w-full">
        <section className="flex items-center gap-3">
          BUSCAR PROFESORES{" "}
          <SecundariaIcon className="w-[2.7rem] mt-[-0.7rem]" />{" "}
        </section>
            ...
            <table className="w-full text-center">
              ...
              <tbody>
                {profesores.map((profesor) => (
                  <tr
                    key={profesor.Id_Profesor_Secundaria}
                    className="border-t border-gray-200"
                  >
                    <td className="py-2 px-2">
                      <div className="w-9 h-9 rounded-full bg-gray-200 mx-auto overflow-hidden">
                        <FotoPerfilClientSide Google_Drive_Foto_ID={profesor.Google_Drive_Foto_ID}/>
                      </div>
                    </td>
                    <td className="py-2 px-2">
                      {profesor.Id_Profesor_Secundaria}
                    </td>
...
export default ProfesoresSecundaria;
